...
import { BrowserLauncher } from "./launch.js";
import { registerReverseTools } from "./reverse.js";
...
class BridgeController {
    ...
    async execute(command, params, signal) {
        ...
        if (!server.status.extensionConnected)
            await this.launcher.ensureConnected();
            ...
            for (const entry of wanted) {
                try {
                    ...
                    if (typeof body.body === 'string') {
                        ...
                        entry.bodyError = body.error;
                    if (typeof body.bytes === 'number')
                        entry.bodyBytes = body.bytes;
                }
                catch (error) {
