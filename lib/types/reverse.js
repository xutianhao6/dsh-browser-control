...
export function registerReverseTools(ctx, host) {
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: args => ({ card: 'generic', title: `CDP ${args.method}`, kind: 'other' }),
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: args => ({ card: 'generic', title: `Cookies ${args.action}`, kind: 'other' }),
        async execute(args, exec) {
            const params = {};
            for (const key of ['url', 'domain', 'name', 'value', 'path', 'sameSite']) {
                if (typeof args[key] === 'string' && args[key].length > 0)
                    params[key] = args[key];
            }
            for (const key of ['secure', 'httpOnly', 'includeHttpOnly', 'values']) {
                if (typeof args[key] === 'boolean')
                    params[key] = args[key];
            }
            for (const key of ['expires', 'limit', 'tabId']) {
                if (typeof args[key] === 'number')
                    params[key] = args[key];
            }
            const map = { get: 'cookies.get', set: 'cookies.set', delete: 'cookies.delete', clear: 'cookies.clear' };
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: args => ({ card: 'generic', title: args.policy ? `Body capture → ${args.policy}` : 'Read body capture policy', kind: 'other' }),
        async execute(args, exec) {
            const params = {};
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: args => ({ card: 'generic', title: args.autoAttach === undefined ? 'List debug targets' : `Targets autoAttach=${args.autoAttach}`, kind: 'other' }),
        async execute(args, exec) {
            let autoAttach;
            if (typeof args.autoAttach === 'boolean') {
                ...
                if (args.tabId !== undefined)
                    params.tabId = args.tabId;
                if (args.waitForDebuggerOnStart === true)
            ...
            if (typeof args.type === 'string')
                listParams.type = args.type;
            if (args.tabId !== undefined)
                listParams.tabId = args.tabId;
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: () => ({ card: 'generic', title: 'Read network body', kind: 'other' }),
        async execute(args, exec) {
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: () => ({ card: 'generic', title: 'Export HAR', kind: 'other' }),
        async execute(args, exec) {
            const params = {};
            if (args.tabId !== undefined)
                params.tabId = args.tabId;
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: () => ({ card: 'generic', title: 'Replay request', kind: 'other' }),
        async execute(args, exec) {
            const params = {};
            if (typeof args.requestId === 'string')
                params.requestId = args.requestId;
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: () => ({ card: 'generic', title: 'Read WebSocket log', kind: 'other' }),
        async execute(args, exec) {
            const params = {};
            for (const key of ['urlPattern', 'payloadPattern', 'direction']) {
                if (typeof args[key] === 'string' && args[key].length > 0)
                    params[key] = args[key];
            }
    ...
    const mapScriptRef = (args) => {
        const params = {};
        if (typeof args.scriptId === 'string')
            params.scriptId = args.scriptId;
        if (typeof args.url === 'string')
            params.url = args.url;
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: args => ({ card: 'generic', title: `Scripts ${args.action}`, kind: 'other' }),
        async execute(args, exec) {
            if (args.action === 'list') {
                const params = {};
                if (typeof args.urlPattern === 'string')
                ...
                if (typeof args.minLength === 'number')
                    params.minLength = args.minLength;
                if (args.withSourceMap === true)
                    params.withSourceMap = true;
                if (args.inlineOnly === true)
                    params.inlineOnly = true;
                if (args.tabId !== undefined)
                    params.tabId = args.tabId;
            ...
            if (args.action === 'dump') {
                ...
                if (typeof args.urlPattern === 'string')
                    listParams.urlPattern = args.urlPattern;
                if (typeof args.minLength === 'number')
                    listParams.minLength = args.minLength;
                if (args.tabId !== undefined)
                    listParams.tabId = args.tabId;
            ...
            if (args.action === 'sourcemap') {
                ...
                if (args.tabId !== undefined)
                    params.tabId = args.tabId;
                const script = await run('scripts.source', { ...params, maxBytes: 1 }, exec.signal);
                if (script.ambiguous === true)
                    return { tabId: Number(script.tabId ?? 0), action: 'sourcemap', js: json({ ambiguous: true, matches: script.matches }) };
                ...
                if (!declared.startsWith('data:')) {
                    try {
                        mapUrl = new URL(declared, scriptUrl || undefined).toString();
                    }
                    catch {
                        mapUrl = declared;
                    }
                ...
                if (mapText === null) {
                    ...
                    const evaluated = await run('eval', { expression, ...(args.tabId === undefined ? {} : { tabId: args.tabId }) }, exec.signal);
                    try {
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: args => ({ card: 'generic', title: `Debugger ${args.action}`, kind: 'other' }),
        async execute(args, exec) {
            const params = {};
            for (const key of ['url', 'urlRegex', 'condition', 'scriptId', 'breakpointId', 'expression', 'callFrameId', 'state']) {
                if (typeof args[key] === 'string' && args[key].length > 0)
                    params[key] = args[key];
            }
            for (const key of ['lineNumber', 'columnNumber', 'timeoutMs', 'tabId']) {
                if (typeof args[key] === 'number')
                    params[key] = args[key];
            }
            if (args.all === true)
            ...
            if (command === undefined)
                throw new Error(`unknown action: ${String(args.action)}`);
            if (args.action === 'step')
                params.action = typeof args.step === 'string' ? args.step : 'over';
            const raw = await run(command, params, exec.signal);
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: args => ({ card: 'generic', title: `Intercept ${args.action}`, kind: 'other' }),
        async execute(args, exec) {
            const params = {};
            for (const key of ['urlPattern', 'stage', 'requestId', 'url', 'method', 'postData', 'responsePhrase', 'errorReason', 'response', 'username', 'password']) {
                if (typeof args[key] === 'string' && args[key].length > 0)
                    params[key] = args[key];
            }
            for (const key of ['limit', 'responseCode', 'tabId']) {
                if (typeof args[key] === 'number')
                    params[key] = args[key];
            }
            for (const key of ['hold', 'handleAuthRequests', 'interceptResponse', 'parked', 'clear']) {
                if (typeof args[key] === 'boolean')
                    params[key] = args[key];
            }
            for (const key of ['patterns', 'headers', 'responseHeaders']) {
                if (args[key] !== undefined)
                    params[key] = args[key];
            }
            if (typeof args.body === 'string')
                params.body = args.body;
            if (typeof args.bodyBase64 === 'string')
                params.bodyBase64 = args.bodyBase64;
    ...
    ctx.tools.register(defineTool({
        ...
        presentCall: args => ({ card: 'generic', title: `Page hook ${args.action}`, kind: 'other' }),
        async execute(args, exec) {
            const params = {};
            if (typeof args.urlPattern === 'string')
                params.urlPattern = args.urlPattern;
