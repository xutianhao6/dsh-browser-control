 ...
 */
export function detectChromePath(explicit: string): string | undefined {
	const configured = expandEnv(explicit.trim())
	if (configured.length > 0) return existsSync(configured) ? configured : undefined
...
export class BrowserLauncher {
	...
	constructor(private readonly options: BrowserLauncherOptions) {}
	...
	private async attempt(config: ResolvedLaunchConfig): Promise<void> {
	...
	private spawnDetached(command: string, args: string[]): void {
		...
		child.on('error', () => { /* reported by the handshake timeout instead */ })
		child.unref()
	}

	/** Poll until the bridge reports a link, or the budget runs out. */
	private async waitForConnection(budgetMs: number): Promise<boolean> {
	 ...
	 */
	private async runBootstrap(script: string, profileDir: string): Promise<void> {
